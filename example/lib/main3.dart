    ...
    return MaterialApp(
      home: Scaffold(
        body: StreamBuilder<BluetoothState>(
          stream: FlutterBluetoothSerial.instance.state.asStream(),
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    StreamBuilder<List<BluetoothDevice>>(
                      stream: FlutterBluetoothSerial.instance.getBondedDevices().asStream(),
                      builder: (context, ss) {
                        if(ss.hasData) {
                          return Column(
                          children: ss.data!.map((device) => ListTile(
                    ...
                    /// Available devices
                    Column(
                      children: scannedDevices.map((device) => ListTile(
                        title: Text(device.device.name.toString()),
                        trailing: TextButton(
                          child: Text("Connect"),
                          onPressed: (){
                            Kbeacon.connect(device.device.address);
                          },
                        ),
                      )).toList()
                    ),
